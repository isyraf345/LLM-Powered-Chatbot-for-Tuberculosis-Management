# config.py (inside app/)

import os

# ============================
# Base paths (IMPORTANT)
# ============================
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MODELS_DIR = os.path.join(BASE_DIR, "..", "models")

# ============================
# DPR MODEL CONFIGURATIONS
# ============================

DPR_MODELS = {
    "single_nq": {
        "ctx": "facebook/dpr-ctx_encoder-single-nq-base",
        "query": "facebook/dpr-question_encoder-single-nq-base",
        "tokenizer": "facebook/dpr-ctx_encoder-single-nq-base",
        "index": os.path.join(MODELS_DIR, "tb_index_single_nq", "tb_faiss.index"),
        "passages": os.path.join(MODELS_DIR, "tb_index_single_nq", "tb_passages.json"),
    },

    "multiset": {
        "ctx": "facebook/dpr-ctx_encoder-multiset-base",
        "query": "facebook/dpr-question_encoder-multiset-base",
        "tokenizer": "facebook/dpr-ctx_encoder-single-nq-base",
        "index": os.path.join(MODELS_DIR, "tb_index_multiset", "faiss.index"),
        "passages": os.path.join(MODELS_DIR, "tb_index_multiset", "passages.json"),
    },

    "lfqa": {
        "ctx": "vblagoje/dpr-ctx_encoder-single-lfqa-wiki",
        "query": "vblagoje/dpr-question_encoder-single-lfqa-base",
        "tokenizer": "facebook/dpr-ctx_encoder-single-nq-base",
        "index": os.path.join(MODELS_DIR, "tb_index_lfqa", "faiss.index"),
        "passages": os.path.join(MODELS_DIR, "tb_index_lfqa", "passages.json"),
    },
}

# ============================
# NLI MODEL
# ============================
NLI_MODEL_NAME = "Bam3752/PubMedBERT-BioNLI-LoRA"

# ============================
# OpenAI
# ============================
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")

if not OPENAI_API_KEY:
    raise RuntimeError(
        "OPENAI_API_KEY is not set. Add it as an environment variable."
    )

OPENAI_MODEL = "gpt-4o-mini"
